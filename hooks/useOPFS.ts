...
export function useOPFSInit(): UseOPFSInitResult {
  const [status, setStatus] = useState<OPFSStatus>(() => opfsService.getStatus());
    ...
    async (
      ...
      try {
        ...
        if (options?.generateThumbnail !== false) {
          ...
          await opfsService.generateThumbnail(savedFile.filePath, transactionId);
        }
...
export function useFileDownload() {
  ...
  const getFile = useCallback(async (filePath: string): Promise<File | null> => {
    setIsLoading(true);
    setError(null);
